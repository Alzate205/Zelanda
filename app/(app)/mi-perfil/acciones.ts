...
import { validarClave } from '@/lib/acceso';
...
export async function actualizarMisDatos(
  ...
  if (usuario.persona_id === null) {
    return {
      ...ESTADO_INICIAL,
      error: 'Tu cuenta no está vinculada a una persona. Pídele al jefe que te asocie.',
    };
  }
...
export async function actualizarMiUsername(
  ...
  if (username !== null) {
    ...
    if (!/^[a-z0-9_.-]+$/.test(username)) {
      return {
        ...ESTADO_INICIAL,
        error: 'Solo letras minúsculas, números y los símbolos _ . - (sin espacios ni @).',
      };
    }
  }
...
export async function cambiarMiContrasena(
  ...
  const vClave = validarClave(nueva, confirmacion);
