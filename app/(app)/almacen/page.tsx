...
export default async function PaginaInicioAlmacen() {
  ...
  const lotesDistintosHoy = new globalThis.Set(ultimasCosechas.map((c) => c.lotes.nombre)).size;
  ...
  const movimientos: MovimientoFila[] = [
    ...ultimasCosechas.map(
      (c): MovimientoFila => ({
        ...
        subtitulo: `${c.metodo_medicion === 'CANASTA' ? 'Canastas' : 'Báscula'} · ${fmtHora(
          c.fecha
        )}`,
        kg: Number(c.peso_kg),
        fecha: c.fecha,
  ...
  return (
    <div className="space-y-5">
      ...
      <section>
        <h2 className="mb-2 font-serif text-base text-zelanda-verde-900">Hoy</h2>
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 rounded-xl border border-l-[3px] border-zelanda-beige-200 bg-white px-3 py-2.5 ${
                    esIngreso ? 'border-l-zelanda-verde-500' : 'border-l-zelanda-ocre-500'
                  }`}
                >
                  <div
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="m-0 text-[13.5px] text-zelanda-verde-900">{m.titulo}</p>
                    <p className="m-0 mt-0.5 text-[11.5px] text-zelanda-verde-700">{m.subtitulo}</p>
                  </div>
