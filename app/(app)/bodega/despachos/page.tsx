...
export default async function PaginaDespachos({
  ...
  const filtroPersona =
    personaIdRaw && /^\d+$/.test(personaIdRaw) ? { persona_id: BigInt(personaIdRaw) } : {};

  const hayFiltros = rango.desde !== null || rango.hasta !== null || personaIdRaw !== '';
  ...
  return (
    <div className="space-y-5">
      <header className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[10.5px] uppercase tracking-[0.18em] text-zelanda-verde-700">Bodega</p>
          <h1 className="mt-1 font-serif text-2xl text-zelanda-verde-900">Despachos</h1>
      ...
      <section className="rounded-2xl border border-zelanda-beige-200 bg-white p-5 shadow-suave">
        <h2 className="flex items-center gap-2 font-serif text-base text-zelanda-verde-900">
          ...
          <span className="text-sm font-normal text-zelanda-verde-700">({abiertos.length})</span>
        </h2>
        {abiertos.length === 0 ? (
          <p className="mt-3 text-sm text-zelanda-verde-700/70">No hay despachos abiertos.</p>
        ) : (
      ...
      <section className="rounded-2xl border border-zelanda-beige-200 bg-white p-5 shadow-suave">
        <h2 className="flex items-center gap-2 font-serif text-base text-zelanda-verde-900">
          <CheckCircle2 className="h-5 w-5 text-zelanda-verde-700" /> {tituloCerrados}{' '}
          <span className="text-sm font-normal text-zelanda-verde-700">({cerrados.length})</span>
        </h2>
        {cerrados.length === 0 ? (
          ...
          <ul className="mt-3 divide-y divide-zelanda-beige-200">
            {cerrados.map((d) => (
              <li key={d.id.toString()} className="py-3 text-sm">
                <p className="font-medium text-zelanda-verde-900">{d.persona.nombre_completo}</p>
                <p className="text-xs text-zelanda-verde-700/70">
                  {hayFiltros ? fmt(d.fecha) : fmtHora(d.fecha)} →{' '}
                  {d.fecha_devolucion &&
                    (hayFiltros ? fmt(d.fecha_devolucion) : fmtHora(d.fecha_devolucion))}{' '}
