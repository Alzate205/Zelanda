...
export function WizardNuevoDespacho({
  ...
  function setQ(id: string, cantidad: number, target: 'herr' | 'ins') {
  ...
  const tituloPorPaso = ['', '¿Para quién?', 'Herramientas e insumos', 'Confirmar despacho'];
        ...
        {filtradas.length === 0 ? (
          ...
          filtradas.map((p) => (
            ...
            >
              ...
              <p className="m-0 flex-1 font-serif text-[15px] text-zelanda-verde-900">{p.nombre}</p>
              {p.id === personaId ? (
                <Check className="h-[18px] w-[18px] text-zelanda-verde-700" strokeWidth={3} />
              ) : null}
            </button>
          ))
        )}
...
function Paso2({
  ...
  return (
    <div>
      <p className="m-0 mb-3 text-[12.5px] text-zelanda-verde-700">
        Despachando para{' '}
        <strong className="font-serif text-zelanda-verde-900">{personaSel?.nombre ?? '—'}</strong>.
      </p>
        ...
        {herramientas.length === 0 ? (
          ...
          herramientas.map((h) => (
            <FilaCantidad
              key={h.id}
              nombre={h.nombre}
              detalle={`${h.disponible} ${h.disponible === 1 ? 'disponible' : 'disponibles'} · ${
                ETIQUETA_CAT[h.categoria]
              }`}
        ...
        {insumosDisponibles.length === 0 ? (
          ...
          insumosDisponibles.map((i) => (
            <FilaCantidad
              key={i.id}
              nombre={i.nombre}
              detalle={`${i.disponible.toLocaleString('es-CO')} ${i.unidad} en stock · ${
                ETIQUETA_CAT[i.categoria]
              }`}
              unidad={i.unidad}
              cantidad={ins[i.id] ?? 0}
              maximo={i.disponible}
              paso={
                i.unidad.toLowerCase().includes('kg') || i.unidad.toLowerCase() === 'l' ? 0.5 : 1
              }
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="m-0 mt-0.5 text-[10.5px] text-zelanda-verde-700">{detalle}</p>
      </div>
      {cantidad === 0 ? (
        <button
          type="button"
...
function Paso3({
  ...
  const asignSel =
    asignacionId !== null ? asignaciones.find((a) => a.id === asignacionId) ?? null : null;
    ...
    <div>
      <div className="overflow-hidden rounded-2xl border border-zelanda-verde-300 bg-white shadow-card">
        <div className="flex items-center gap-2.5 bg-gradient-to-br from-zelanda-verde-700 to-zelanda-verde-800 px-3.5 py-3 text-zelanda-beige-50">
          <AvatarIniciales id={personaSel.id} nombre={personaSel.nombre} tamano="md" />
          <div className="min-w-0 flex-1">
          ...
          {herrItems.length > 0 ? (
            <>
              ...
              <ul className="mb-3 space-y-1">
                {herrItems.map((it) => (
                  <li key={it.id} className="flex items-center justify-between text-[13px]">
