...
export default async function PaginaHistorialInsumo({
  ...
  const disponible =
    Number(insumo.stock_actual) - Number(insumo.stock_reservado);
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-zelanda-beige-200 bg-white p-5 shadow-suave">
        ...
        <p className="mt-1 text-xs text-zelanda-verde-700/70">
          Sumá (positivo) o restá (negativo) cuando un insumo se rompe, se
          pierde, o el conteo físico no coincide.
        </p>
        <div className="mt-3">
            ...
            {movimientos.map((m) => {
              ...
              return (
                <li key={m.id.toString()} className="py-3 text-sm">
                  ...
                  {m.notas && (
                    <p className="mt-1 text-xs italic text-zelanda-verde-700">
                      {m.notas}
                    </p>
                  )}
                </li>
