  ...
  return (
    <div className="space-y-5">
      <header>
        ...
        <h1 className="mt-1 font-serif text-2xl text-zelanda-verde-900">Inventario de bodega</h1>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {insumos.map((i) => (
                ...
                >
                  <td className="py-2 pr-3 font-medium text-zelanda-verde-900">{i.nombre}</td>
                  <td className="py-2 pr-3 text-zelanda-verde-700">{i.categoria}</td>
                  <td className="py-2 pr-3 text-right">
                    {formatearCantidad(i.stock_actual)} {i.unidad}
                  </td>
                  <td className="py-2 pr-3 text-right">{formatearCantidad(i.stock_reservado)}</td>
                  <td
                    className={`py-2 pr-3 text-right font-medium ${
                      i.por_debajo_minimo ? 'text-estado-vencida' : ''
                    }`}
                  >
                    {formatearCantidad(i.stock_disponible)}
                  </td>
                  <td className="py-2 pr-3 text-right">{formatearCantidad(i.stock_minimo)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="rounded-2xl border border-zelanda-beige-200 bg-white p-5 shadow-suave">
        ...
        <ul className="mt-3 divide-y divide-zelanda-beige-200">
          {herramientas.map((h) => (
            <li key={h.id.toString()} className="grid grid-cols-[1fr_auto_auto] gap-3 py-2 text-sm">
