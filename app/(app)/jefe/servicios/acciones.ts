...
const ESTADOS_VALIDOS: estado_servicio[] = ['ACUERDO', 'EN_CURSO', 'TERMINADO', 'CANCELADO'];
...
async function crearPersonaBasica(nombreCompleto: string): Promise<bigint> {
  const rows = await prisma.$queryRaw<{ id: bigint }[]>`
    INSERT INTO personas (nombre_completo, activo, created_at, updated_at)
    VALUES (${nombreCompleto}, true, NOW(), NOW())
    RETURNING id
  `;
  return rows[0].id;
}
...
export async function crearServicio(
  ...
  const personaIdExistente = parsearId(String(formData.get('persona_id') ?? ''));
  const nuevoContratistaNombre = String(formData.get('nuevo_contratista_nombre') ?? '').trim();
