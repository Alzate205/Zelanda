...
export default async function MapaArbolesLote({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="-mx-4 -mt-6 space-y-5">
      <div className="bg-gradient-to-b from-zelanda-verde-800 to-zelanda-verde-700 px-4 pb-4 pt-3 text-zelanda-beige-50">
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-[12px] text-zelanda-beige-100/85">
          ...
          {conNovedad > 0 ? (
            <span>
              <strong className="font-serif text-sm text-white">{conNovedad}</strong> con novedad
            </span>
          ) : null}
        </div>
      </div>
            ...
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {estadosOrden.map((est) => (
                  ...
                  >
                    <span className={`h-2.5 w-2.5 rounded-sm ${COLOR_LEYENDA[est]}`} aria-hidden />
                    {ETIQUETA_ESTADO[est]}
                  </span>
                ))}
              </div>
