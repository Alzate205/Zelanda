    ...
    <div className="-mx-4 -mt-6 space-y-5">
      ...
      >
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-[12px] text-zelanda-beige-100/85">
          <span>
            <strong className="font-serif text-sm text-white">{apiario.total_colmenas}</strong>{' '}
            colmenas
          </span>
      ...
      <div className="flex flex-wrap gap-2 px-4">
        ...
        {!apiario.activo ? <BadgeBase tono="alerta">Inactivo</BadgeBase> : null}
      </div>
      ...
      <div className="px-4">
        <section className="rounded-2xl border border-zelanda-beige-200 bg-white p-5 shadow-suave">
          ...
          <dl className="mt-3 space-y-3 text-sm">
            <div className="flex items-center gap-3">
              ...
              <dt className="w-24 text-[10.5px] uppercase tracking-[0.12em] text-zelanda-verde-700">
                Colmenas
              </dt>
            ...
            <div className="flex items-start gap-3">
              ...
              <dt className="w-24 shrink-0 text-[10.5px] uppercase tracking-[0.12em] text-zelanda-verde-700">
                Ubicación
              </dt>
          ...
          <ul className="mt-3 divide-y divide-zelanda-beige-200">
            {filasTarea.map((f) => (
              <li key={f.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
                ...
                <div className="flex items-center gap-2">
                  <BadgeBase tono={tonoBadge(f.estado)}>{etiquetaEstado(f.estado)}</BadgeBase>
                  <Link
        ...
        <section className="rounded-2xl border border-zelanda-beige-200 bg-white p-5 shadow-suave">
          <h2 className="font-serif text-base text-zelanda-verde-900">Últimas visitas</h2>
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm text-zelanda-verde-900">
                        {v.fecha_registro.toLocaleString('es-CO', {
                          ...
                          hour: '2-digit',
                          minute: '2-digit',
                          // Render en servidor (UTC): sin zona explícita la hora sale 5h adelantada.
                          timeZone: 'America/Bogota',
                        })}
                      </p>
                      <p className="text-xs text-zelanda-verde-700">{v.persona.nombre_completo}</p>
                    </div>
                    <BadgeBase tono={tonoEstadoApiario(v.estado_apiario)}>
                      {v.estado_apiario ? ETIQUETA_ESTADO_APIARIO[v.estado_apiario] : 'Sin estado'}
                    </BadgeBase>
                  </div>
                  {v.observaciones ? (
                    <p className="mt-1 text-xs text-zelanda-verde-700/80">{v.observaciones}</p>
                  ) : null}
                </li>
              ))}
            </ul>
                ...
                <li key={String(c.id)} className="py-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-zelanda-verde-700">
                        {c.fecha.toLocaleDateString('es-CO', {
                          day: '2-digit',
                          month: 'short',
                          year: 'numeric',
                        })}
