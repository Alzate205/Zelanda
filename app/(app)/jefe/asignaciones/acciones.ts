...
export async function crearAsignacionesMasivas(input: {
  ...
  if (input.destinos.some((d) => d.kind !== kindEsperado)) {
    return {
      error: `Los destinos deben ser ${kindEsperado === "lote" ? "lotes" : "apiarios"} para esta tarea.`,
      creadas: 0,
      duplicadas: 0,
    };
  }
  ...
  const ocupados = new Set(
    abiertas.map((a) =>
      kindEsperado === "lote" ? String(a.lote_id) : String(a.apiario_id),
    ),
  );
  ...
  try {
    ...
    if (destinatario) {
      ...
      await enviarPushAUsuarios([destinatario.id], {
        ...
        cuerpo: `${tipo.nombre} · ${creadas.length} ${kindEsperado === "lote" ? "lotes" : "apiarios"}`,
        url: "/trabajador",
