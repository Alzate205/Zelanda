...
export async function crearPago(_prev: EstadoPago, formData: FormData): Promise<EstadoPago> {
  ...
  if (Math.abs(monto) > 500_000_000) {
    return { error: 'Monto fuera de rango (máximo 500.000.000 COP).' };
  }
...
export async function editarPago(_prev: EstadoPago, formData: FormData): Promise<EstadoPago> {
  const usuario = await requerirUsuario('JEFE');

  const id = parsearId(String(formData.get('id') ?? ''));
  if (!id) return { error: 'Pago no encontrado.' };
  ...
  if (cubreDesdeRaw || cubreHastaRaw) {
    ...
    if (cubreHasta < cubreDesde) return { error: 'Hasta no puede ser anterior a Desde.' };
  }
  ...
  try {
    ...
    return { error: sanitizarError(e, 'pagos/editar') };
  }
