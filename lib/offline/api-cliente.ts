...
export async function enviarSalida(payload: PayloadSalida): Promise<Resultado> {
  ...
  return intentarSubirGenerico('/api/almacen/salida', { ...payload, id_local }, 'salida', id_local);
}
...
async function intentarSubirGenerico(
  ...
  try {
    ...
    if (clase === 'permanente') {
      ...
      return { ok: false, error: err, enCola: false };
    }
    // El servidor falló, no la señal. Antes se devolvía "guardado" y el
    // trabajador seguía a la pantalla de éxito creyendo que quedó registrado,
    // mientras el registro se quedaba dando vueltas en la cola sin que nadie
    // se enterara. Decir la verdad es lo que hace visible el problema.
    return {
      ok: false,
      error:
        'El servidor no pudo guardarlo ahora. Quedó pendiente y se reintenta solo; revísalo en Pendientes.',
      enCola: true,
    };
  } catch {
    // Sin poder distinguir "se cayó la red" de "no hay señal": queda en cola.
    return { ok: true, offline: true, id_local };
  }
}
